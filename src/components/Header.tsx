      ...
      <div className="flex justify-between items-center w-full px-8 py-4 max-w-7xl mx-auto">
        <Link href="/" className="flex items-center gap-2">
          {settings.logoUrl ? (
            ...
            <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">{settings.siteTitle || "دروب القمة"}</div>
          )}
        </Link>
        ...
        <div className="flex gap-4 items-center">
          <a className="hidden sm:flex flex-col text-right" href={`tel:${settings.phone || "966583165533"}`}>
            <span className="text-[10px] text-slate-400 font-bold">اتصل الآن</span>
            <span className="font-public-sans text-sm font-semibold tracking-tight text-blue-900 dark:text-blue-300" dir="ltr">{settings.phone || "966583165533"}</span>
          </a>
          <a href={settings.ctaLink || "tel:966583165533"} className="bg-primary text-on-primary px-6 py-2 rounded-lg font-cta text-cta hover:opacity-90 transition-opacity flex items-center gap-2">
